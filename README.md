...
Odrant Point ID = UUID клиента (`crypto.randomUUID()`), хранится и как Qdrant point id, и в `payload.id`. Векторное поле — заглушка `[0.0]` (dim=1); при добавлении семантического поиска заменяется реальным вектором без изменения схемы.

### StorageBackend (интерфейс)

Весь `storageModule` работает только через этот интерфейс. Смена бэкенда = подстановка другой реализации.

```
v1: QdrantBackend  — REST API к Qdrant Cloud
v2: LocalBackend   — IndexedDB (тот же интерфейс, без сети)
```
